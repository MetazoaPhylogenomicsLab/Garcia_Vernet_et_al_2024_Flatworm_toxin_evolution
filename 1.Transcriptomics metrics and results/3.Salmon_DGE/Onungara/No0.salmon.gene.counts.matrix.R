...
if (file.exists("No0.salmon.gene.counts.matrix.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("No0.salmon.gene.counts.matrix.RData")
} else {
    print('Reading matrix file.')
    primary_data = read.table("../6.Salmon/No0.salmon.gene.counts.matrix", header=T, com='', row.names=1, check.names=F, sep='\t')
    primary_data = as.matrix(primary_data)
}
source("/home/raquelgv/trinityrnaseq-2.11.0/Analysis/DifferentialExpression/R/heatmap.3.R")
source("/home/raquelgv/trinityrnaseq-2.11.0/Analysis/DifferentialExpression/R/misc_rnaseq_funcs.R")
source("/home/raquelgv/trinityrnaseq-2.11.0/Analysis/DifferentialExpression/R/pairs3.R")
source("/home/raquelgv/trinityrnaseq-2.11.0/Analysis/DifferentialExpression/R/vioplot2.R")
data = primary_data
...
write.table(data, file="No0.salmon.gene.counts.matrix.minRow10.CPM.log2.centered.dat", quote=F, sep='	');
pdf("No0.salmon.gene.counts.matrix.minRow10.CPM.log2.centered.prcomp.principal_components.pdf")
prin_comp_data = data
...
write.table(pca$rotation, file="No0.salmon.gene.counts.matrix.minRow10.CPM.log2.centered.PCA.prcomp.scores", quote=F, sep="	")
write.table(pca$x, file="No0.salmon.gene.counts.matrix.minRow10.CPM.log2.centered.PCA.prcomp.loadings", quote=F, sep="	")
PCA.loadings=pca$x
